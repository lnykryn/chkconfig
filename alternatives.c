 ...
 *
 * When MERGED_SBIN is enabled, identifies alternatives that differ only in
 * bin vs sbin paths and removes duplicates, preferring /bin/ over /sbin/.
 * Modifies the set in place, adjusting best and current indices as needed.
 *
    ...
    for (i = 0; i < set->numAlts; i++) {
        for (j = i + 1; j < set->numAlts; j++) {
            ...
            if (streq_bin(set->alts[i].leader.target, set->alts[j].leader.target)) {
                ...
                /* Keep the /bin/ variant, remove the /sbin/ one */
                if (strncmp(set->alts[i].leader.target, "/sbin/", 6) == 0 ||
                    strncmp(set->alts[i].leader.target, "/usr/sbin/", 10) == 0) {
                    if (FL_VERBOSE(flags)) {
